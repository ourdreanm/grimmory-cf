...
export async function hashPassword(p:string){const salt=crypto.getRandomValues(new Uint8Array(16));const k=await crypto.subtle.importKey("raw",enc.encode(p),"PBKDF2",false,["deriveBits"]);const b=new Uint8Array(await crypto.subtle.deriveBits({name:"PBKDF2",salt,iterations:120000,hash:"SHA-256"},k,256));return `pbkdf2$120000$${b64(salt)}$${b64(b)}`}
export async function verifyPassword(p:string,s:string){const z=s.split("$");if(z.length!==4)return false;const k=await crypto.subtle.importKey("raw",enc.encode(p),"PBKDF2",false,["deriveBits"]);const b=new Uint8Array(await crypto.subtle.deriveBits({name:"PBKDF2",salt:unb64(z[2]),iterations:Number(z[1]),hash:"SHA-256"},k,256)),e=unb64(z[3]);if(b.length!==e.length)return false;let d=0;for(let i=0;i<b.length;i++)d|=b[i]^e[i];return d===0}
